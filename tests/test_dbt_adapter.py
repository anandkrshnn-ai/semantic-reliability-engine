...
def test_dbt_adapter_parse_extended_generic_tests(tmp_path):
    schema_yaml = """
    version: 2
    models:
      - name: stg_orders
    ...
    suite = DBTTestAdapter.parse_schema_yml(f, model_name="stg_orders")
    assert len(suite.assertions) == 3
    types = [a.assertion_type for a in suite.assertions]
    assert "accepted_values" in types
    assert "accepted_range" in types
