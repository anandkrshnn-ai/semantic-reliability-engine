...
class SurvivingDefectTaxonomy:
    ...
    KNOWN_HOLDOUT_SURVIVING_DEFECTS: List[SurvivingDefectRecord] = [
        SurvivingDefectRecord(
            ...
            root_cause_code="POPULATION_FILTER_UNCONSTRAINED",
            description="Inclusive testing shift survived as equivalent bounds-wise without a RequiredPopulationAssertion on the channel join key.",
            missing_contract_dimension="cohort_eligibility",
            recommended_assertion="RequiredPopulationAssertion(source_table='ad_campaign_performance', required_filter='is_test_campaign = false', join_key='channel')",
